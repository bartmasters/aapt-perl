#!/pkgs/bin/perl -w
######################################################
# fixso.pl		Fix BIOSS Service Order attributes
#
...
# Service Type from fixso.cfg file
...
# the attribute matches the pattern.
		    ...
		    defined($attributepattern))
		{
			...
			elsif ($attributepattern eq '?^(256k\/64k|512k\/128k|1.5M\/256k)$')
			{
				fixrate($attributevalue);
			}
			elsif ($attributepattern eq ":YN")
			{
				fixyesno($attributevalue);
			}
			elsif ($attributepattern eq '?^(NA|827|827 4V|1720 + WIC|1750 + WIC)$')
			{
				fixcpe($attributevalue);
			}
		}
	}
}
...
# dd-mm-yyyy.
	...
	if ($olddate =~ /^\d{1,2}\/\d{2}\/\d{1,2}$/)
	{
		my $newdate = $olddate;
		$newdate =~ s/\//-/g;

		if (substr($newdate, 2,1) eq "-")
		{
			substr($newdate, 6,2) += 2000;
		}
		else
		{
			substr($newdate, 5,2) += 2000;
		}
...
sub fixrate
{
# I could be smart and put in pattern matching - but I decided that 
# the big hammer was quicker and easier.  So sue me.

	my $oldrate = shift;
	my $newrate;

	if ($oldrate eq "1.5MEG/256K")
	{
		$newrate = "1.5M/256k";
	}
	elsif ($oldrate eq "1.5m/256k")
	{
		$newrate = "1.5M/256k";
	}
	elsif ($oldrate eq "1500/256")
	{
		$newrate = "1.5M/256k";
	}
	elsif ($oldrate eq "1500kbps/256kbps")
	{
		$newrate = "1.5M/256k";
	}
	elsif ($oldrate eq "256/64")
	{
		$newrate = "256k/64k";
	}
	elsif ($oldrate eq "512/128")
	{
		$newrate = "512k/128k";
	}
	else
	...
	updatedb($newrate, $oldrate, $serviceorderid, $attributename);
}
# fixyesno - Fix yes/no. 
#
# Parameters:	None
...
sub fixyesno
{
	...
	updatedb($newyes, $oldyes, $serviceorderid, $attributename);
}
# fixcpe - Fix the cpe. 
#
# Parameters:	None
...
sub fixcpe
{
	...
	updatedb($newcpe, $oldcpe, $serviceorderid, $attributename);
}
...
sub mailadmin
{
	...
	$mail->open({
		...
		"To"		=> $adminmailaddr,
		"Subject"	=> "Problem running fixso.pl"
			})
