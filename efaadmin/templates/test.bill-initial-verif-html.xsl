<?xml version="1.0" ?>
<!-- $Id: test.bill-initial-verif-html.xsl,v 1.9 2005/03/30 07:50:00 bamaster Exp $ -->
...
<!-- we use xsl:text to preserve the blank line, otherwise <xsl:output method="html"> will strip it -->
		<html>
			...
			<p>All you need to do now is verify that we have your correct email address. You can do this simply by going to: <a href="{action_page}?{verify_action}&amp;{customer_key}"><xsl:value-of select="action_page"/>?<xsl:value-of select="verify_action"/>&amp;<xsl:value-of select="customer_key"/>
			</a></p>
